...
const Tab = ({ path, index, label, currentPath }: { path: string; index: number; label: string; currentPath: string }) => {
    ...
    <nav className="flex items-center justify-between w-full px-2 py-1 bg-crust border-strong font-mono text-xs sm:text-sm select-none overflow-x-auto custom-scrollbar">
      ...
      <div className="flex items-center space-x-2 sm:space-x-4 shrink-0">
        ...
        <div className="flex items-center space-x-1">
          <Tab path="/" index={0} label="ranks" currentPath={location.pathname} />
          {user && <Tab path="/settings" index={1} label="config" currentPath={location.pathname} />}
        </div>
      </div>
        ...
        {user ? (
          <div className="flex items-center space-x-2">
            ...
            <Link
              ...
              className="flex items-center space-x-2 text-lavender hover:text-mauve transition-colors group max-w-[110px] sm:max-w-[200px]"
            >
              {user.avatarUrl ? (
                <img
                  src={user.avatarUrl}
            ...
            <button onClick={signOut} className="hover:text-red transition-colors">
              logout
            </button>
          </div>
        ) : (
          <button onClick={handleLogin} className="text-lavender hover:text-mauve transition-colors whitespace-nowrap">
            &lt;login_discord&gt;
          </button>
        )}
      </div>
