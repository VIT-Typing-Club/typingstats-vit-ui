...
export default function PublicProfilePage() {
  ...
  useEffect(() => {
    ...
    const convertToAscii = () => {
      ...
      img.onload = () => {
        ...
        for (let i = 0; i < pixels.length; i += 4) {
          ...
          const charIndex = Math.floor(
            (luminance / 255) * (ASCII_DENSITY.length - 1),
          )
      ...
      img.onerror = () => {
        console.warn(
          "Could not load image for ASCII conversion due to CORS or bad URL.",
        )
      }
    ...
    return (
      <div className="flex h-full bg-base p-6 font-mono text-sm text-subtext0">
        <span className="animate-pulse">
          &gt; fetching user_data for '{username}'...
        </span>
      </div>
    )
  }
      ...
      <div className="max-w-3xl w-full mx-auto flex flex-col sm:flex-row items-center sm:items-start gap-8 sm:gap-12 mt-4 sm:mt-12">
        ...
        <div className="flex flex-col w-full min-w-0">
          ...
          <div className="flex flex-col space-y-1.5">
            <FetchRow
              label="Display"
              value={profile.displayName || profile.discordUsername}
            />
            ...
            <FetchRow
              ...
              linkUrl={
                profile.mtUrl
                  ? `https://monkeytype.com/profile/${profile.mtUrl}`
                  : undefined
              }
            ...
            <FetchRow
              ...
              isLink
              linkUrl={`https://github.com/${profile.githubUrl}`}
            />
            <FetchRow
              label="LinkedIn"
              value={profile.linkedinUrl}
              isLink
              linkUrl={`https://linkedin.com/in/${profile.linkedinUrl}`}
            />
            <FetchRow
              label="X"
              value={profile.xUrl}
              isLink
              linkUrl={`https://x.com/${profile.xUrl}`}
            />
            <FetchRow
              label="Instagram"
              value={profile.instagramUrl}
              isLink
              linkUrl={`https://instagram.com/${profile.instagramUrl}`}
            />
          </div>

          <div
            className="mt-6 flex space-x-0 cursor-default select-none"
            aria-hidden="true"
          >
